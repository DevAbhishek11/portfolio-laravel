...
@section('content')
    ...
    <div class="bg-slate-800/50 border border-slate-700 rounded-2xl p-4 mb-6 backdrop-blur-sm shadow-sm">
        <div class="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-4" id="projects-filter">
            <div class="relative md:col-span-2">
                ...
                <input type="text" id="projects-search" value="{{ request('search') }}"
            ...
            <div class="flex items-center gap-2">
                <button type="button" id="projects-reset"
                    class="flex-1 px-4 py-2 bg-rose-500/10 text-rose-500 hover:bg-rose-500/20 text-sm font-bold rounded-xl transition-all">
                    Reset
                </button>
            </div>
        </div>
    ...
    @push('scripts')
        <script>
            (function () {
                ...
                const statusSelect = document.getElementById('projects-status');
                ...
                function buildAjaxUrl() {
                    const url = new URL("{{ route('admin.projects.ajax') }}", window.location.origin);
                ...
                function buildPublicUrl() {
                    const url = new URL("{{ route('admin.projects.index') }}", window.location.origin);
                    if (state.search) url.searchParams.set('search', state.search);
                    if (state.status) url.searchParams.set('status', state.status);
                    if (state.category) url.searchParams.set('category', state.category);
                ...
                wrapper.addEventListener('click', (e) => {
                    const pageLink = e.target.closest('#projects-pagination a');
                    if (pageLink) {
                        e.preventDefault();
